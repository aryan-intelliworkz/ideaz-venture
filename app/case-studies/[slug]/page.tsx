...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: cs ? `${cs.title} — Ideaz Ventures` : "Case Study — Ideaz Ventures",
    description: cs?.description || "Explore our successful projects",
  };
}

export default async function CaseStudyDetailPage({ params }: Props) {
  const { slug } = await params; // ← THIS WAS MISSING! Must have await
          ...
          <div className="flex flex-col gap-10 lg:gap-12 xl:gap-[60px]">
            <div className="flex flex-col lg:flex-row gap-10 lg:gap-6 xl:gap-[32px]">
              <div className="flex items-start gap-4 shrink-0">
                ...
                <h2 className="font-archivo font-medium text-[28px] md:text-[32px] lg:text-[32px] xl:text-[36px] 2xl:text-[38px] leading-[1.2] text-white capitalize max-w-[510px]">
                  {cs.vision.heading}
                </h2>
              </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-y-10">
              {cs.stats.map((stat, i) => (
                ...
                >
                  <div className="flex flex-col gap-4">
                    <span className="font-archivo font-medium text-[28px] md:text-[32px] lg:text-[32px] xl:text-[36px] 2xl:text-[38px] leading-[1.09] text-white capitalize">
                      {stat.value}
                    </span>
                    <div className="h-[1px] w-full bg-gray-400" />
                  </div>
